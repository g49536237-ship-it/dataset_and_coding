...
# 划分训练集和测试集
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=24)   # 最后保存随机数为24

# 定义参数网格
grid_params = {
    'max_iter': [300, 500, 800, 1000],
    'tol': [1e-3, 1e-4, 1e-5],
    'alpha_1': [1e-5, 1e-6, 1e-8],
    'alpha_2': [1e-5, 1e-6, 1e-8],
    'lambda_1': [1e-5, 1e-6, 1e-8],
    'lambda_2': [1e-5, 1e-6, 1e-8],
    'compute_score': [True]
}

# 创建贝叶斯岭回归模型对象
base_model = BayesianRidge()
grid_search = GridSearchCV(base_model, grid_params, cv=5, scoring='neg_mean_squared_error', n_jobs=-1)
grid_search.fit(X_train, y_train)

print("最佳参数:", grid_search.best_params_)

# 用最佳参数创建模型
model = BayesianRidge(**grid_search.best_params_)
...
plt.scatter(y_pred_test, y_test, color='#00FFFF', marker='o', alpha=0.9, label='Test Data Set', edgecolor='black', s=70,
            linewidth=0.5)

# 绘制95%置信区间
plt.fill_between([y_test.min(), y_test.max()],
                 [test_slope * y_test.min() + test_intercept - test_conf_band,
                  test_slope * y_test.max() + test_intercept - test_conf_band],
                 [test_slope * y_test.min() + test_intercept + test_conf_band,
                  test_slope * y_test.max() + test_intercept + test_conf_band],
                 color='#FFA500',
                 alpha=0.5,
...
# 绘制95%预测区间
plt.fill_between([y_test.min(), y_test.max()],
                 [test_slope * y_test.min() + test_intercept - test_pred_band,
                  test_slope * y_test.max() + test_intercept - test_pred_band],
                 [test_slope * y_test.min() + test_intercept + test_pred_band,
                  test_slope * y_test.max() + test_intercept + test_pred_band],
                 color='#90EE90',
                 alpha=0.3,
...
plt.text(0.60, 0.02, f'Fitting equation:\ny = {test_slope:.2f}x + {test_intercept:.2f}\nR$^2$: {test_r2:.2f}',
         fontsize=25, color='black', ha='left', va='bottom', fontname='Arial', fontweight='normal',
...
# 保存测试集拟合图像
plt.savefig(r"D:\pytharm\shijie\picture\BRR-test_fit.png", dpi=600)
...
plt.scatter(y_pred_train, y_train, color='#00BFFF', marker='o', alpha=0.9, label='Train Data Set', edgecolor='black',
            s=70, linewidth=0.5)

# 绘制95%置信区间
plt.fill_between([y_train.min(), y_train.max()],
                 [train_slope * y_train.min() + train_intercept - train_conf_band,
                  train_slope * y_train.max() + train_intercept - train_conf_band],
                 [train_slope * y_train.min() + train_intercept + train_conf_band,
                  train_slope * y_train.max() + train_intercept + train_conf_band],
                 color='#F08080',
                 alpha=0.5,
...
# 绘制95%预测区间
plt.fill_between([y_train.min(), y_train.max()],
                 [train_slope * y_train.min() + train_intercept - train_pred_band,
                  train_slope * y_train.max() + train_intercept - train_pred_band],
                 [train_slope * y_train.min() + train_intercept + train_pred_band,
                  train_slope * y_train.max() + train_intercept + train_pred_band],
                 color='#FFDAB9',
                 alpha=0.3,
...
# 保存训练集拟合图像
plt.savefig(r"D:\pytharm\shijie\picture\BRR-train_fit.png", dpi=600)
