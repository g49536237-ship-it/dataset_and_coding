...
# 读取数据集
data = pd.read_csv('3-2.csv', encoding='gbk')  # 请确保该文件存在
...
# 调整后的参数网格以平衡模型性能，避免过拟合和欠拟合
grid_params = {
    'n_estimators': [100, 150],  # 增加树的数量以提高模型性能
    'max_depth': [10, 15],       # 提高树的最大深度以捕获更多特征
    'min_samples_split': [3],    # 调整分裂所需的最小样本数
    'min_samples_leaf': [1],     # 减少叶节点的最小样本数以提高灵活性
    'max_features': ['sqrt']
}
...
# 使用网格搜索寻找最佳参数（cv 降为3，加快速度）
grid_search = GridSearchCV(estimator=et, param_grid=grid_params, cv=3, n_jobs=-1, verbose=1, scoring='r2')
...
print("训练集 MSE: %.4f" % mean_squared_error(y_train, y_pred_train))
...
plt.grid(False)
plt.tight_layout()
plt.savefig(r"D:\pytharm\shijie\picture\Extra Trees-test_fit.png", dpi=600)
# plt.show()
...
plt.savefig(r"D:\pytharm\shijie\picture\Extra Trees-train_fit.png", dpi=600)
# plt.show()
